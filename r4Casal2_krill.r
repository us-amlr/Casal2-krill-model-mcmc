...
rm(list=ls())
wd <- ''              # working directory for mcmc chains
ch.names <- c('0K.a','0K.b','0K.c')           # directories containing identically configured mcmc chains
n.ch <- length(ch.names)                      # number of chains
current.chain <- '0K.c'                       # chain to be plotted
wd2 <- paste(wd,current.chain,'/biom/',sep='') # directory for Casal2 outputs to be plotted
setwd(wd)

#lib.path <- 'C:/zot/Casal2/2023/CASAL2_Alistair/R-libraries/'
lib.path <- 'C:/zot/Casal2/2023/R-Libraries_Casal2_Alistair/Casal2_23.09/'
path <- paste(lib.path,'casal2/R/',sep='')
r.scrpts <- dir(path)
...
# script below assumes three mcmc chains were run
...
ssbs <- cbind(ssbs,B0=rep(cas2_tab$summary$values[[1]],47)) # add B0 column to ssbs 
                                                            # (47 years of B0 values*111 mcmc samples)
...
median(cas2_tab$summary$values[[1]])  # B0 mcmc median value for the chain
